...
class WebMessageMapperTest {
	...
	void toWebMessageFromWebMessageEntity() {
		...
		// Setup
		final var fileData_1 = "bytes-1".getBytes();
		final var fileData_2 = "bytes-2".getBytes();
		final var webMessageEntity = WebMessageEntity.create()
			.withAttachments(List.of(
				AttachmentEntity.create().withFile(fileData_1).withFileName("file-1.txt").withId(1).withMimeType("text/plain"),
				AttachmentEntity.create().withFile(fileData_2).withFileName("file-2.txt").withId(2).withMimeType("text/plain")
			))
		...
		.containsExactly(
			Tuple.tuple("file-1.txt", new String(encodeBase64(fileData_1)), "text/plain"),
			Tuple.tuple("file-2.txt", new String(encodeBase64(fileData_2)), "text/plain"));
		...
		// Call toWebMessages (reusing same webMessageEntity used in test of method toWebMessage)for verification of list of messages
